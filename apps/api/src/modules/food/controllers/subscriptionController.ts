...
export const createSubscription = async (req: Request, res: Response) => {
  try {
    ...
    if (addons && addons.length > 0) {
      addonsTotal = addons.reduce((sum: number, addon: any) => {
...
export const pauseSubscription = async (req: Request, res: Response) => {
  try {
    ...
    subscription.pauseReason = reason || 'Paused by user';
...
export const modifySubscription = async (req: Request, res: Response) => {
  try {
    ...
    if (addons) {
      ...
      const newAddonsTotal = addons.reduce((sum: number, addon: any) => {
...
export const getTodaysDeliveries = async (req: Request, res: Response) => {
  try {
    ...
    const groupedBySlot = activeDeliveries.reduce((acc: any, sub) => {
