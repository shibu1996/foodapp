...
export const performanceMonitor = (req, res, next) => {
  ...
  res.json = function (data) {
    ...
    if (duration > SLOW_REQUEST_THRESHOLD) {
      ...
      console.warn(
        `🐌 SLOW REQUEST: ${endpoint} took ${duration}ms | IP: ${req.ip} | User: ${req.userId || 'anonymous'}`
      );
 ...
 */
const trackSlowEndpoint = (endpoint, duration) => {
 ...
 */
export const trackQueryTime = (queryName) => {
  const startTime = Date.now();
