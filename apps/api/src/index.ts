...
import { connectDatabase, getConnectionStatus } from './shared/config/database';
import { connectRedis, getRedisStatus } from './shared/config/redis';
import { cacheService } from './shared/services/cacheService';
import authRoutes from './shared/routes/authRoutes';
import productRoutes from './modules/food/routes/productRoutes';
import categoryRoutes from './modules/food/routes/categoryRoutes';
import orderRoutes from './modules/food/routes/orderRoutes';
import subscriptionRoutes from './modules/food/routes/subscriptionRoutes';
...
import { performanceMonitor, getPerformanceMetrics } from './shared/middleware/performanceMonitor';
app.use(performanceMonitor);

// Input validation and sanitization (prevents injection attacks)
import { sanitizeBody, sanitizeQuery, antiInjection } from './shared/middleware/validation';
...
app.get('/health/queues', async (req, res) => {
  try {
    const { getQueuesHealth } = await import('./shared/queue/queueConfig');
    const { getWorkersStatus } = await import('./shared/queue/workers');
...
const startServer = async () => {
  ...
  try {
    ...
    if (redisAvailable) {
      try {
        const { startQueueWorkers } = await import('./shared/queue/workers');
        startQueueWorkers();
      } catch (queueError) {
